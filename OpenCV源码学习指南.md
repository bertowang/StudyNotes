...
14. [几何变换算法](#14-几何变换算法)
    - [14.1 仿射变换](#141-仿射变换)
    - [14.2 重映射（Remap）](#142-重映射remap)
15. [特征检测算法](#15-特征检测算法)
    - [15.1 ORB 特征检测器](#151-orb-特征检测器)
...
ORB (Oriented FAST and Rotated BRIEF) 是 OpenCV 主力特征检测器。

```
class ORB_Impl : public ORB
{
    // 1. 使用 FAST 检测关键点
    std::vector<KeyPoint> detect(const Mat& img) {
        // 多尺度 FAST 检测
    }
    
    // 2. 计算方向（Oriented）
    void computeOrientation(const Mat& img, std::vector<KeyPoint>& kpts) {
        // 使用灰度质心法计算方向
    }
    
    // 3. 计算 BRIEF 描述子（Rotated）
    Mat computeDescriptors(const Mat& img, const std::vector<KeyPoint>& kpts) {
        // 根据关键点方向旋转 BRIEF 模式
    }
};
```
