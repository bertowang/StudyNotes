...
- [11. 按 demo 流程拆解：每个节点的输入和输出](#11-按-demo-流程拆解每个节点的输入和输出)
- [12. Decoder 和 Encoder 的差别](#12-decoder-和-encoder-的差别)
- [13. 训练 Transformer 是个什么过程？](#13-训练-transformer-是个什么过程)
- [14. 推理（生成）阶段是什么样？](#14-推理生成阶段是什么样)
- [15. 常见问题答疑（FAQ）](#15-常见问题答疑faq)
- [16. 学习路线建议](#16-学习路线建议)
- [17. 一图总结](#17-一图总结)
- [18. 基础 Transformer 的演进思路](#18-基础-transformer-的演进思路)
- [19. 写在最后](#19-写在最后)
...
为了让逻辑更接近“先完整 demo，再逐节点拆解”的学习方式，本文可以按下面这条主线阅读：

```text
先理解 Transformer 要解决什么问题
→ 看整体架构和零件清单
→ 逐个理解 6 个核心零件
→ 在第 10 章把零件拼成最小 Encoder demo
→ 按 10.1 节的节点表回看每个零件的输入、处理和输出
→ 再理解 Decoder / GPT、训练、推理和现代演进
```

也就是说，第 4～9 章是“零件说明书”，第 10 章是“完整参考 demo”，第 10.1 节是“把 demo 和零件一一对应起来”。

| 学习阶段 | 重点问题 | 对应章节 |
|---|---|---|
| 先看整体 | Transformer 是什么，为什么比 RNN/CNN 更适合长距离依赖？ | 第 0～3 章 |
| 拆零件 | Embedding、位置编码、Attention、MHA、FFN、Add & Norm 分别做什么？ | 第 4～9 章 |
| 看 demo | 一个最小 Encoder 如何把这些零件串起来？ | 第 10 章 |
| 对节点 | 每个处理节点的输入、输出、对应函数是什么？ | 第 10.1 节 |
| 扩展到 GPT | Encoder 和 Decoder / GPT 有什么差别？ | 第 11 章 |
...
## 11 按 demo 流程拆解：每个节点的输入和输出

这一节对应前面的 `TransformerEncoder` demo。它的作用类似一张“流程对照表”：把完整代码拆成一个个处理节点，说明每个节点的**输入是什么、处理做了什么、输出是什么、应该回看哪一章**。

#### 节点 1：输入 token id `tokens`
...
| 项目 | 内容 |
|---|---|
| 输入 | 离散的 token id，形状是 `(B, L)`。 |
| 示例 | `(2, 16)` 表示一次输入 2 句话，每句话 16 个 token。 |
| 处理 | 只是准备模型输入，此时每个 token 还是整数编号。 |
| 输出 | `tokens`。 |
| 回看章节 | 第 4 章 Token Embedding。 |

初学者要记住：模型真正吃进去的不是原始文字，而是 tokenizer 处理后的 token id。本文 demo 用随机整数模拟真实 token id。

#### 节点 2：词嵌入 `self.embedding(tokens)`
...
|---|---|
| 输入 | `tokens`，形状 `(B, L)`。 |
| 处理 | 通过 `nn.Embedding` 查表，把每个 token id 映射成一个 `d_model` 维向量。 |
| 输出 | `x`，形状 `(B, L, d_model)`。 |
| 回看章节 | 第 4 章。 |

这一步完成了从“离散编号”到“连续向量”的转换。后面的 Attention、FFN 都是在向量上计算。
...
| 项目 | 内容 |
|---|---|
| 输入 | token 向量 `x` 和位置编码 `pe`。 |
| 处理 | 截取当前序列长度对应的位置编码，并加到 token embedding 上。 |
| 输出 | 带位置信息的 `x`，形状仍是 `(B, L, d_model)`。 |
| 回看章节 | 第 5 章。 |

如果没有位置编码，Self-Attention 本身不知道 token 的先后顺序；加入位置编码后，模型才能区分“我打你”和“你打我”。
...
|---|---|
| 输入 | 加过位置编码的 `x`。 |
| 处理 | 训练时随机丢弃一部分特征，降低过拟合风险。 |
| 输出 | 形状不变的 `x`。 |
| 回看章节 | 第 9 章和第 10 章。 |

Dropout 不改变张量形状，只是在训练阶段增加一点“扰动”，让模型不要死记硬背。

#### 节点 5：进入第 1 到第 N 个 `EncoderLayer`
...
|---|---|
| 输入 | `x`，形状 `(B, L, d_model)`。 |
| 处理 | 串联经过 N 个 `EncoderLayer`。每一层都包含 Self-Attention、FFN、残差连接和 LayerNorm。 |
| 输出 | 上下文感知后的 `x`，形状仍是 `(B, L, d_model)`。 |
| 回看章节 | 第 6～10 章。 |

这里最关键的一点是：N 层是**串联**，不是并联。每一层的 shape 不变，但每个 token 向量里的语义信息会越来越丰富。

#### 节点 6：Self-Attention 让 token 彼此交流
...
| 项目 | 内容 |
|---|---|
| 输入 | 当前层的 `x`，以及由 `x` 线性变换得到的 `Q`、`K`、`V`。 |
| 处理 | 每个 token 和其他 token 计算相关度，再按权重汇总信息。 |
| 输出 | 融合上下文后的 attention 输出。 |
| 回看章节 | 第 6 章。 |

Self-Attention 是 Transformer 的核心：它让每个位置都能直接“看见”其他位置，因此更容易处理长距离依赖。

#### 节点 7：Multi-Head Attention 从多个视角看句子
...
|---|---|
| 输入 | 当前层的 `x`。 |
| 处理 | 把注意力拆成多个 head 并行计算，再拼接回 `d_model`。 |
| 输出 | 多视角融合后的 attention 输出。 |
| 回看章节 | 第 7 章。 |

可以把多头注意力理解成多个专家同时读同一句话：有的看语法关系，有的看指代关系，有的看局部搭配。

#### 节点 8：FFN 对每个 token 单独深加工
...
|---|---|
| 输入 | Attention 之后的 `x`。 |
| 处理 | 对每个位置独立执行“升维 → 激活 → 降维”。 |
| 输出 | 形状仍为 `(B, L, d_model)` 的新表示。 |
| 回看章节 | 第 8 章。 |

Attention 负责“和别人交流”，FFN 负责“自己加工”。两者配合，才构成完整的 Transformer 层。

#### 节点 9：Add & Norm 保持信息和训练稳定
...
|---|---|
| 输入 | 子层输入和子层输出。 |
| 处理 | 残差连接保留原信息，LayerNorm 稳定数值分布。 |
| 输出 | 稳定后的 `x`。 |
| 回看章节 | 第 9 章。 |

没有残差连接，深层网络容易越学越难；没有归一化，训练过程容易数值不稳定。
...
| 项目 | 内容 |
|---|---|
| 输入 | 最后一层 `EncoderLayer` 的输出。 |
| 处理 | 直接作为 Encoder 的最终输出返回。 |
| 输出 | `out`，形状 `(B, L, d_model)`。 |
| 回看章节 | 第 10 章。 |

这个输出不是最终文字，而是每个 token 的上下文感知向量。后续可以接分类头、抽取头、MLM 头，或者作为 Encoder-Decoder 架构中 Decoder 的参考信息。

#### 整个 demo 的数据流总结

| 流程节点 | 输入 | 处理函数 / 模块 | 输出 |
|---|---|---|---|
| 准备输入 | 原始文本经过 tokenizer 后的 token id | `tokens` | `(B, L)` |
| 词嵌入 | `tokens` | `nn.Embedding` | `(B, L, d_model)` |
| 加位置 | token 向量 + `pe` | `positional_encoding`、`+` | `(B, L, d_model)` |
| 正则化 | `x` | `Dropout` | `(B, L, d_model)` |
| 堆叠编码层 | `x` | `EncoderLayer × N` | `(B, L, d_model)` |
| 注意力交流 | `Q`、`K`、`V` | `scaled_dot_product_attention`、`MultiHeadAttention` | attention 输出 |
| 单点加工 | attention 后的 `x` | `FeedForward` | `(B, L, d_model)` |
| 稳定训练 | 子层输入和输出 | `Add & Norm` | `(B, L, d_model)` |
| 最终输出 | 最后一层输出 | `return x` | `out` |
...
> Transformer Encoder demo 的主线就是：**先把 token id 变成带位置的向量，再让这些向量反复经过 Attention 交流、FFN 加工、Add & Norm 稳定，最后得到上下文感知向量。**

---

## 12. Decoder 和 Encoder 的差别
...
### 12.1 带 Mask 的自注意力（Masked Self-Attention）
...
### 18.7 演进方向六：训练范式 & 对齐 —— 从"会接龙"到"听人话"
